...
import { SEAT_STYLES } from "../../lib/constants";
...
export default function SeatPicker({
  ...
  const getSeatStyle = (seat: SeatState | undefined): React.CSSProperties => {
    ...
    if (seat.selected) {
      return {
        ...
        cursor: "pointer",
        transform: "scale(1.1)",
        boxShadow: "0 0 8px rgba(45, 138, 78, 0.2)",
      };
    }
  ...
  const handleReserveClick = async () => {
    ...
    await onReserve(selectedSeats.map((s) => s.id));
    // Reset after reserve
    setSeatGrid(grid.map((r) => r.map((s) => ({ ...s, selected: false }))));
    setSelectedSeats([]);
  };

  const reverseSeatGrid = [...seatGrid].reverse();

  console.log("reverse", reverseSeatGrid);
    ...
    <div className='flex flex-col gap-6'>
      ...
      <section>
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, transparent, var(--color-border), var(--color-border-subtle), transparent)",
          }}
        />
        <p
          className='text-center text-xs text-muted tracking-widest uppercase'
      ...
      <section className='overflow-x-auto'>
        <div className='flex justify-center'>
          <div className='inline-block min-w-max mx-auto'>
            {reverseSeatGrid.map((row, rowIdx) => {
      ...
      <section>
        <div className='flex flex-wrap gap-4 justify-center'>
          {(
            Object.entries(SEAT_STYLES) as [
              keyof typeof SEAT_STYLES,
              (typeof SEAT_STYLES)[keyof typeof SEAT_STYLES],
            ][]
          ).map(([type, style]) => {
            ...
            return (
              ...
              >
                <div
                  className='w-5 h-5 rounded-t-xs'
                  style={{
                    background: style.available.bg,
                    border: `1px solid ${style.available.border}`,
                  }}
